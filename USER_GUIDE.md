...
The generated report is an interactive, slide-style deck that you navigate with on-screen arrows or your keyboard. The tool can also package the report's underlying per-vehicle diagnostic data as CSV files for validation.
...
When generation completes, the **Report Ready** screen summarises the result and offers four actions.
...
- **Download All (ZIP)** — saves the report's underlying diagnostic data as a ZIP of four CSV files, named like `Fleet Insights_<DATABASE>_<YYMMDD-YYMMDD>_diagnostics.zip` (see [Exporting Diagnostic CSVs](#exporting-diagnostic-csvs)).
...
On the **Report Ready** screen, click **Download All (ZIP)** to download the raw per-vehicle data behind the report as a single ZIP archive — useful for validation or deeper analysis in a spreadsheet. The archive contains four CSV files:

| File | Contents |
|--------|----------|
| **`…_vehicles.csv`** | Device ID, Device Name, Group, Trip Count, Active Days, Distance (km), Drive Hours, Idle Hours, Idle Cost, Safety Score, Safety Events, Battery Faults |
| **`…_safety_events.csv`** | Device ID, Device Name, Rule ID, Rule Name, Date Time, Duration |
| **`…_battery_faults.csv`** | Device ID, Device Name, Diagnostic ID, Diagnostic Name, Code, Date Time |
| **`…_engine_faults.csv`** | Device ID, Device Name, Diagnostic ID, Diagnostic Name, Code, Controller ID, Date Time |

The diagnostic CSVs are generated by the tool and are **not** embedded in the shared HTML report, which keeps the report file small. They are available from the **Report Ready** screen for as long as the tool keeps the finished report — by default about **2 hours** after generation (the `JOB_TTL_HOURS` setting). To retrieve them later, generate the report again.

> 🖼️ **[Screenshot: Report Ready screen showing the Download All (ZIP) button]** — `docs/screenshots/20-diagnostic-csv.png`
