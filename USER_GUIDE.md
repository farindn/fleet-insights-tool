...
- **Download Raw Data** — saves the report's underlying diagnostic data as a ZIP of four CSV files, named like `Fleet Insights_<DATABASE>_<YYMMDD-YYMMDD>_diagnostics.zip` (see [Exporting Diagnostic CSVs](#exporting-diagnostic-csvs)).
...
On the **Report Ready** screen, click **Download Raw Data** to save the raw per-vehicle data behind the report as a single ZIP archive — useful for validation or deeper analysis in a spreadsheet. The archive contains four CSV files:
...
> 🖼️ **[Screenshot: Report Ready screen showing the Download Raw Data button]** — `docs/screenshots/20-diagnostic-csv.png`
